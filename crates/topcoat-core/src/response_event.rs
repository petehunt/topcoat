...
impl Cx {
    fn send_json_with_key(&self, key: String, json: String) -> Result<JsonKey> {
        let mut state = self.inner.response_events.state();
        ...
        self.inner.response_events.push(
            &mut state,
            ResponseEvent::Json {
                key: key.clone(),
                json,
    ...
    pub fn has_response_events(&self) -> bool {
        self.inner.response_events.has_events()
    }
    ...
    pub fn take_response_event_receiver(&self) -> ResponseEventReceiver {
        ResponseEventReceiver {
            events: std::sync::Arc::clone(&self.inner.response_events),
            receiver: self.inner.response_events.take_receiver(),
        }
    }
}
...
mod tests {
    ...
    fn named_json_is_deduplicated_and_conflicts_are_rejected() {
        ...
        assert!(matches!(
            cx.inner.response_events.pop(),
            Some(ResponseEvent::Json { key, .. }) if key == "products"
        ));
        assert!(cx.inner.response_events.pop().is_none());
    }
